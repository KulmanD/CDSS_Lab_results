...
Use the frontend at `http://127.0.0.1:5173/`, choose one of these CSV files, and submit it through the upload form.

To try PDF upload, create a text-based PDF from one of these CSV files. On macOS, open the CSV in a text editor, choose Print, then Save as PDF. The PDF upload path only supports readable text that still contains these CSV-compatible rows; scanned/image PDFs are not supported.
